...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | The provider used when no explicit provider is specified. Flip with
    | PRISM_PROVIDER=anthropic to switch the live provider without any code
    | change.
    |
    */
    'default_provider' => env('PRISM_PROVIDER', 'fireworks'),
    ...
    'providers' => [
        ...
        */
        'anthropic' => [
            ...
            // Default model used when no model is passed to ::using().
        ...
        |--------------------------------------------------------------------------
        |
        | Fireworks exposes an OpenAI-compatible inference API. Prism's built-in
        | OpenAI driver is reused here via the custom PrismManager extension
        | registered in TelegramAgentServiceProvider. The provider key must be
        | 'fireworks' so that config('prism.providers.fireworks') resolves.
        |
        */
        'fireworks' => [
            'driver' => 'openai',
            ...
            'project' => null,
            // Default model used when no model is passed to ::using().
            'default_model' => env('FIREWORKS_MODEL', 'accounts/fireworks/models/minimax-m2p7'),
        ],
