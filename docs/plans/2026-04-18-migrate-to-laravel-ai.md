...
## Done — results (2026-04-18)

- LA1 `9fdc3c9` — 5 tools rewritten as `Laravel\Ai\Contracts\Tool` (-LOC custom interface, +LOC framework contract).
- LA2 `6d16850` — TelegramAgent on Promptable + RemembersConversations (+RedisConversationStore, -AgentSession/AgentSessionStore).
- LA3 `2f46d8b` — webhook controller rewired via per-request factory construction.
- LA4 `5d5befe` — obsolete LLM client stack deleted: AnthropicToolUseClient + FireworksToolUseClient + ToolUseClientContract + ToolDefinition + ToolUseResult + AgentTool + AgentContext + AgentSession + AgentSessionStore + HandlesTelegramUpdate + 2 tests.
- LA5 `(this commit)` — live smoke passed: 4 818 ms wall time, 1 Fireworks call → HTTP 200, 1 Telegram sendMessage (faked). Model replied in Vietnamese explaining workflow capabilities without immediately invoking RunWorkflowTool. No ExecutionRun created on first free-text message (expected: model explained rather than triggered on ambiguous prompt). Config fix required: `fireworks` provider driver changed from `openai` (Responses API — incompatible) to `groq` (Chat Completions — compatible).

**Driver note:** `laravel/ai`'s built-in `openai` driver targets the OpenAI Responses API (`/responses`). Fireworks only supports Chat Completions (`/chat/completions`). Using the `groq` driver with a custom `url` is the correct approach for any OpenAI-compatible Chat Completions endpoint.

**Tool names in API calls:** `laravel/ai` Groq gateway sends `class_basename($tool)` as the function name (`RunWorkflowTool`, `ListWorkflowsTool`, etc. — PascalCase). The model understands these names correctly.

**Total test count after migration:** 102 tests passing.
**Net LOC change (plan commit → LA4):** +1 334 insertions / −2 541 deletions (net −1 207 LOC).
