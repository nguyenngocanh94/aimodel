...
- LA5 `c2bc5e8` — live smoke passed: 4 818 ms wall time, 1 Fireworks call → HTTP 200, 1 Telegram sendMessage (faked). Model replied in Vietnamese explaining workflow capabilities without immediately invoking RunWorkflowTool. No ExecutionRun created on first free-text message (expected: model explained rather than triggered on ambiguous prompt). Config fix required: `fireworks` provider driver changed from `openai` (Responses API — incompatible) to `groq` (Chat Completions — compatible).
