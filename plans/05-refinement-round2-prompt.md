# Refinement Round 2 Prompt for GPT Pro

> Fresh conversation. Extended Reasoning ON. Paste everything below the line.

---

Below is a design plan for an AI Video Workflow Builder. It has been through multi-model synthesis and one refinement round. It is approximately 95% complete.

Your job: review the ENTIRE plan with extreme care. Assume there are at least 30 remaining issues you need to find. Do not stop after finding 10-15 problems — keep going.

For each issue found, provide:
1. What the problem is (be specific)
2. Why it matters
3. Git-diff style fix

## Categories to examine

### Contradictions
- Do types defined in one section match how they're used in another?
- Does the boot sequence align with the persistence model?
- Do the execution engine types match the run store types?
- Does the node catalog match the type compatibility matrix?

### Underspecified Interactions
- What happens when a user deletes a node that's currently running?
- What happens when a user edits config while a run is in progress?
- What happens when autosave fires during an active run?
- How does the template system interact with the node registry? (Are templates validated against the current registry version?)
- How does "Run from here" interact with the review checkpoint node?

### Missing Error States
- What does the UI show when IndexedDB is in degraded/memory-fallback mode?
- What happens when workflow JSON import has a valid structure but semantically broken edges?
- What does the storyboard player show when upstream image generation mock produces no images?

### Testing Gaps
- Are there tests specified for the boot state machine?
- Are there tests for the retention/GC policy?
- Are there tests for the BroadcastChannel multi-tab warning?
- Are there tests for the edge insertion command + undo?

### Performance Concerns
- How many nodes before React Flow performance degrades?
- How large can a WorkflowDocument get before Dexie writes become slow?
- What's the maximum payload size before the Data Inspector becomes sluggish?

### Consistency
- Is naming consistent? (camelCase vs kebab-case in file names, etc.)
- Are all node types in the catalog also present in the file structure?
- Do all keyboard shortcuts have corresponding store commands?
- Does the validation model cover every edge case mentioned in the journeys?

## After fixing issues

Once you've found and fixed everything, provide:
1. A numbered list of ALL changes made
2. Your honest assessment: is this plan ready for implementation, or does it need another round?
...
[Then paste the FULL contents of plans/04-refinement-round1-result.md here, preceded by this header:]
